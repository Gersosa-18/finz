...
def post_crear_alerta_simple(alerta: AlertaSimpleCreate, user_id: int = 1, db: Session = Depends(get_db)):
...
def post_crear_alerta_rango(alerta: AlertaRangoCreate, user_id: int = 1, db: Session = Depends(get_db)):
...
def post_crear_alerta_porcentaje(alerta: AlertaPorcentajeCreate, user_id: int = 1, db: Session = Depends(get_db)):
...
def post_crear_alerta_compuesta(alerta: AlertaCompuestaCreate, user_id: int = 1, db: Session = Depends(get_db)):
...
@alertas_router.get('/alertas/usuario/{user_id}', tags=['Alertas'])
...
@alertas_router.get('/alertas/activadas/{user_id}', tags=['Alertas'])
